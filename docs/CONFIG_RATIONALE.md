...
| `kernel.sched_child_runs_first` | `1` | `1` | After `fork()`, the child runs before the parent | Makes fork-based race conditions slightly harder by reducing the window where the parent can manipulate shared state before the child executes. Low cost, no compatibility impact. | Setting to `0` (default) is not a security risk per se, but slightly widens the race window. | Kicksecure |
